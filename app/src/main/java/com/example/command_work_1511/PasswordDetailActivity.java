...
public class PasswordDetailActivity extends AppCompatActivity {
    ...
    private void checkPinAndDeletePassword(String enteredPin, long passwordId) {
        ...
        String savedHashedPin = sharedPreferences.getString("hashed_pin", null);  // Убедитесь, что ключ совпадает
        ...
        // Хэшируем введенный PIN для сравнения
        String enteredHashedPin = EncryptionUtils.hashPassword(enteredPin.trim());

        if (enteredHashedPin != null && savedHashedPin.equals(enteredHashedPin)) {
